...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figs/Agent_Skills.pdf}
    \vspace{-0.2in}
...
Since a loaded \emph{skill} persists across an entire session, its influence may extend beyond its declared scope. For example, a medical diagnosis \emph{skill}  may make an agent more conservative or disclaimer-prone even when it later performs an unrelated task such as code generation. This is not a conflict between multiple \emph{skills} or a failure within the \emph{skill}'s intended scope, but a form of \textbf{behavioral spillover}. Since in-context instructions can shape subsequent outputs~\cite{brown2020language} and prompt format alone can shift accuracy substantially~\cite{sclar2024prompt}, a loaded \emph{skill} may alter the agent’s behavior beyond its intended boundaries. How to detect, quantify, and suppress such spillover is an open question.
Simply declaring scope boundaries is unlikely to be sufficient, because a \emph{skill}'s boundaries are themselves just instructions interpreted by the model, not hard execution constraints.
...
Existing work studies model drift and prompt sensitivity largely in isolation, but \emph{skill} portability is harder because multiple factors interact simultaneously: models may interpret the same instruction differently, required tools may be unavailable, and the target model's knowledge may either overlap with or fall short of what the \emph{skill} assumes. Avoid-lists and tool dependencies require model-specific adaptation: an instruction to ``avoid tool X'' is meaningless if the model does not know X exists. Thus, portability is not literal output matching, but \emph{contract satisfaction}: correct behavior, predictable failure modes, and reliable fallback across diverse settings.
...
While open-source governance offers useful lessons~\cite{ostrom1990commons,eghbal2016roads}, skills raise additional concerns because their encoded knowledge may be uniquely scarce and irreplaceable. Sixty percent of open-source maintainers are currently unpaid hobbyists~\cite{tidelift2024maintainer}. Among inactive PyPI developers, 75.4\% never issue deprecation notices~\cite{zhong2025deprecation}.  This makes abandonment, privacy, and liability especially important for \emph{skills}. A central open question is how to govern \emph{skills} in ways that sustain valuable expertise while preserving openness, accountability, and long-term access.
...
Evaluating a skill is difficult for the same reason that evaluating online recipes is difficult: many alternatives exist, but the right criteria depend on the task, the user’s goals, and the context. Skills make this harder because performance varies widely across domains~\cite{skillsbench2026}, so aggregate metrics are not enough. Meaningful evaluation likely requires domain-specific task suites, failure taxonomies, and success criteria, yet these depend on the same tacit expertise that makes skills valuable. Existing dynamic and community-driven benchmarks~\cite{kiela2021dynabench,srivastava2023beyond,jimenez2024swebench} provide useful precedent, but skills add two challenges: evaluation is tightly tied to domain authorship, and benchmarks may be gamed as skills improve. The open question is how to build evaluation frameworks that are domain-grounded, robust to gaming, and sustainable over time.
...
The rapid growth of the skills ecosystem—from a handful of curated packages to over 400,000 public Skills in a matter of months—signals genuine demand. Yet the infrastructure needed to make this ecosystem reliable, safe, and sustainable does not yet exist. We believe that if the research community addresses the open questions outlined in Section 4, the impact on AI agents will unfold across two horizons.
In the near term, agents equipped with high-quality, composable \emph{skills} will achieve reliable expert-level execution in specialized domains that raw model scaling cannot reach: a clinical agent that follows verified diagnostic workflows rather than hallucinating procedures, a synthesis planning agent that executes feasible chemical routes rather than plausible-sounding ones, a financial agent that applies precise domain logic rather than approximate reasoning. The gains come not from a larger model, but from procedural expertise loaded on demand.

In the longer term, the \emph{skills} paradigm opens a more ambitious possibility: agents that do not merely consume \emph{skills}, but create, evaluate, and evolve them~\cite{skillrl2026,yang2026autoskill}. An agent that completes a complex task can distill that experience into a reusable \emph{skill}; an agent that observes \emph{skill} failures can revise the procedure; a community of agents can collectively improve a shared \emph{skill} library over time. This self-evolving loop—where agents become both users and authors of procedural knowledge—represents a qualitative shift from static tool use toward a living, self-improving knowledge infrastructure. 
...
\vspace{-0.05in}
\paragraph{\textbf{The Skill developer's perspective}} The ask is straightforward: follow emerging standards when authoring \emph{skills}, and share them openly. A \emph{skill} that is well-scoped, clearly documented, and contributed to a public registry does not just help one agent;  it becomes reusable infrastructure for many agents that follows. The more developers treat \emph{skills} as a \textbf{shared knowledge commons} rather than isolated proprietary artifacts, the more quickly the ecosystem can accumulate value.

\vspace{-0.05in}
\paragraph{\textbf{The Skill user's perspective}} 
The ask is to treat \emph{skills} as a first-class mechanism for extending agent capability, learn how to select and compose them effectively, and provide feedback when they fail. When a user reports that a \emph{skill} behaves unexpectedly, interferes with adjacent tasks, or breaks under certain conditions, that feedback improves not just one interaction but the reliability of the broader ecosystem.
...
% \vspace{+0.1in}
Whether \emph{skills} ultimately become a lasting systems layer or only a transitional engineering pattern remains an open question. What is already clear, however, is that they force the community to confront a deeper issue: how \textbf{procedural knowledge should be represented, shared, evaluated, and governed in agentic AI}. We hope this paper serves as an invitation to study that question seriously. By doing so, the field has an opportunity not only to make agents more capable, but also to make them more understandable, reusable, and trustworthy in the specialized workflows where they are most needed.
